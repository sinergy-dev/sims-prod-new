...
@section('content')
  ...
  <!--  -->
    <div class="box">
        ...
        <h6 class="box-title"><i class="fa fa-table"></i>&nbspDetail ID Project</h6>
        ...
        <div class="box-body">
            <a href="{{url('/salesproject')}}"><button class="btn btn-danger margin-bottom" style="width: 150px"><i class="fa fa-home">&nbspBack to Home</i></button></a>
                ...
                @if (session('success'))
                  <div class="alert alert-success notification-bar"><span>Notice : </span> {{ session('success') }}<button type="button" class="dismisbar transparant pull-right"><i class="fa fa-times fa-lg"></i></button><br>Get your PID :<h6> {{$pops->id_project}}</h6></div>
                @elseif (session('error'))
                  <div class="alert alert-danger notification-bar" id="alert"><span>notice: </span> {{ session('error') }}.</div>
                @endif
                    <span>
                      <h6>ID Project Induk : <b>{{$induk->id_project}}</b></h6>
                    </span>
                    <span>
    ...
    <div class="modal-dialog modal-md">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h6 class="modal-title">Add ID Project</h6>
        </div>
