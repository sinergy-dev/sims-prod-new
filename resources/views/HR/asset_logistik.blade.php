      ...
      <div class="card-body">
        <div class="nav-tabs-custom active" id="asset" role="tabpanel">
          <ul class="nav nav-tabs" id="myTab" role="tablist">
            <li class="nav-item">
              <a class="nav-link active" id="profile-tab" data-bs-toggle="tab" href="#list_asset" role="tab" aria-controls="kategori" aria-selected="false">List Asset</a>
            </li>
            <li class="nav-item">
              <a class="nav-link home-tab" id="home-tab" data-bs-toggle="tab" style="display: none;" href="#peminjaman_asset_logistik" role="tab" aria-controls="home" aria-selected="true">Request Logistik - Facilities</a>
            </li>
            <li class="nav-item">
              <a class="nav-link home-tab-2" id="home-tab-2" data-bs-toggle="tab" style="display: none;" href="#peminjaman_asset_logistik_2" role="tab" aria-controls="home" aria-selected="true">Request Logistik - Requestor</a>
            </li>
            <!-- @if(Auth::User()->id_division == 'HR')
            <li class="nav-item">
              <a class="nav-link" id="home-tab" data-bs-toggle="tab" href="#request_pr" role="tab" aria-controls="home" aria-selected="true">Request PR</a>
            </li>
            @endif -->
            <div class="form-group ms-auto" >
              <button class="btn btn-sm btn-success tambah_asset_logistik" data-bs-toggle="modal" id="tambah_asset_logistik" data-bs-target="#add_asset" style="display: none!important"><i class="bx bx-plus"> </i>&nbsp Asset</button>
              <button class="btn btn-sm btn-warning btnExport" id="btnExport" style="display: none!important;" onclick="exportExcel('{{action('AssetLogistikController@reportExcel')}}')"><i class="bx bx-download"> </i>&nbsp Excel</button>
              <div class="dropdown dropdownBln" style="display: none;">
                ...
                <ul class="dropdown-menu" id="selectShowMonth">
                  @foreach($month_formatted as $value)
                    <li class="dropdown-item" style="cursor:pointer; padding-top: 5px;padding-bottom: 5px;text-align: center;" onclick="reportExcel(this)">{{$value}}</li>
                  @endforeach
                </ul>
              </div>
              <div class="dropdown">
                <button class="btn btn-sm btn-success request_logistik" id="request_logistik" style="margin-right: 5px;" style="display: none!important;"><i class="bx bx-plus"> </i>&nbspRequest Logistik</button>
                <div class="dropdown-content">
                  <a data-bs-toggle="modal" data-bs-target="#peminjaman_modal">Available</a>
                  <a data-bs-toggle="modal" data-bs-target="#request_modal">Unavailable</a>
                </div>
              </div>
            </div>
          </ul>
          <div class="tab-content" id="myTabContent">
            <div class="tab-pane show active home-tab" id="list_asset" role="tabpanel" aria-labelledby="home-tab">
              <br>
                  ...
                  <tbody id="products-list" name="products-list">
                    ...
                    <tr>
                      ...
                      <td id="col_action_2" class="col_action" style="display: none;">
                        <a href="{{url('/asset_logistik/detail_asset_logistik', $data->id_barang) }}"><button class="btn btn-sm btn-primary" style="width:35px;height:30px;border-radius: 25px!important;outline: none;"><i class="bx bx-history" aria-hidden="true" data-bs-toggle="tooltip" title="History" data-placement="bottom"></i></button></a>
            ...
            <div class="tab-pane fade" id="peminjaman_asset_logistik" role="tabpanel" aria-labelledby="profile-tab">
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        @elseif($data->status == 'REJECT')
                          <button class=" btn btn-sm btn-danger" data-bs-target="#reject_note_modal" data-bs-toggle="modal" style="width: 90px; color: white;" onclick="reject_note('{{$data->id_transaction}}', '{{$data->note}}')"> REJECTED</button>
                      ...
                      <td>
                        @if($data->status == 'PENDING')
                        <button class="btn btn-sm btn-success" id="btn_accept" name="btn_accept" value="{{$data->id_transaction}}" style="width: 90px; height: 25px;" onclick="id_accept_update('{{$data->id_transaction}}','{{$data->id_barang}}', '{{$data->qty}}', '{{$data->qty_akhir}}', '{{$data->nama_barang}}', '{{$data->keterangan}}', '{{$data->nik_peminjam}}', '{{$data->created_at}}')">ACCEPT</button>
                        <button class="btn btn-sm btn-danger" id="btn_reject" name="btn_reject" value="{{$data->id_transaction}}" style="width: 90px; height: 25px;"  onclick="reject_update('{{$data->id_transaction}}')">REJECT</button>
                        @elseif($data->status == 'PROSES')
                        <button class="btn btn-sm btn-primary" id="btn-done" data-bs-target="#done_modal" data-bs-toggle="modal" name="btn_done" value="{{$data->id_transaction}}" style="width: 90px; height: 25px" onclick="update_done_pr('{{$data->id_transaction}}', '{{$data->id_barang}}', '{{$data->qty}}', '{{$data->qty_request}}', '{{$data->nama_barang}}')">DONE</button>
                        @elseif($data->status == 'DONE')
                        <button class="btn btn-sm btn-primary disabled" style="width: 90px; height: 25px">DONE</button>
                        @else
                      ...
                      <tr>
                        ...
                        <td>
                          ...
                          @elseif($data->status == 'ACCEPT')
                            <label class="label label-primary" style="width: 90px">ACCEPTED</label>
                          @elseif($data->status == 'REJECT')
                            <button class=" btn btn-sm btn-danger" data-bs-target="#reject_note_modal" data-bs-toggle="modal" style="width: 90px; color: white;" > REJECTED</button>
                          @endif
                        </td>
                        <td>
                          @if($data->status == 'REQUEST')
                          <button class="btn btn-sm btn-success" id="btn_accept_request_logistik" value="{{$data->id_barang}}" name="btn_accept" style="width: 90px; height: 25px;">ACCEPT</button>
                          <button class="btn btn-sm btn-danger" id="btn_reject" name="btn_reject" style="width: 90px; height: 25px;" onclick="reject_request_logistik('{{$data->id_barang}}')">REJECT</button>
                          @elseif($data->status == 'PROCESS')
                          <button class="btn btn-sm btn-primary" id="btn-done" data-bs-target="#done_request_modal" data-bs-toggle="modal" name="btn_done" style="width: 90px; height: 25px" onclick="done_request_logistik('{{$data->id_barang}}', '{{$data->nama}}', '{{$data->qty}}', '{{$data->nik}}', '{{$data->keterangan}}')">DONE</button>
                          @elseif($data->status == 'DONE')
                          <button class="btn btn-sm btn-primary" style="width: 90px; height: 25px">DONE</button>
                          @else
                          <button class="btn btn-sm btn-success disabled" style="width: 90px; height: 25px;">ACCEPT</button>
                          <button class="btn btn-sm btn-danger disabled" style="width: 90px; height: 25px;">REJECT</button>
                          @endif
            ...
            <div class="tab-pane fade" id="peminjaman_asset_logistik_2" role="tabpanel" aria-labelledby="profile-tab">
              <div class="table-responsive" style="margin-top: 15px">
                  ...
                  <tbody id="products-list" name="products-list">
                    ...
                    @foreach($pr_request as $data)
                      <tr>
                        ...
                        <td>
                          @if($data->status == 'PENDING')
                          <button class="btn btn-sm btn-success" id="btn_accept" name="btn_accept" value="{{$data->id_transaction}}" style="width: 90px; height: 25px;" data-bs-target="#accept_modal" data-bs-toggle="modal" onclick="id_accept_update('{{$data->id_transaction}}','{{$data->id_barang}}', '{{$data->qty}}', '{{$data->qty_akhir}}', '{{$data->created_at}}')">ACCEPT</button>
                          <button class="btn btn-sm btn-danger" id="btn_reject" name="btn_reject" value="{{$data->id_transaction}}" style="width: 90px; height: 25px;" data-bs-target="#reject_modal" data-bs-toggle="modal" onclick="id_reject_update('{{$data->id_transaction}}','{{$data->id_barang}}', '{{$data->qty}}', '{{$data->qty_akhir}}')">REJECT</button>
                          @elseif($data->status == 'PROSES')
                          <button class="btn btn-sm btn-primary" id="btn-done" data-bs-target="#done_modal" data-bs-toggle="modal" name="btn_done" value="{{$data->id_transaction}}" style="width: 90px; height: 25px" onclick="update_done_pr('{{$data->id_transaction}}', '{{$data->id_barang}}', '{{$data->qty}}', '{{$data->qty_request}}', '{{$data->nama_barang}}')">DONE</button>
                          @elseif($data->status == 'DONE')
                          <button class="btn btn-sm btn-primary disabled" style="width: 90px; height: 25px">DONE</button>
    ...
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-header">
          <h6 class="modal-title">Add Asset Logistik</h6>
        </div>
        <div class="modal-body">
          <form method="POST" action="{{url('asset_logistik/store_asset_logistik')}}" name="modalProgress">
            @csrf
          ...
          <div class="form-group">
            <label>Unit</label>
            <select class="form-control unit_logistik" name="unit" id="unit"style="width: 100%" required>
...
<div class="modal fade" id="modaledit" role="dialog">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-header">
          <h6 class="modal-title">Update Asset</h6>
        </div>
        <div class="modal-body">
          <form method="POST" action="{{url('asset_logistik/edit_logistik')}}" name="modalProgress">
            @csrf
...
<div class="modal fade" id="modalrestock" role="dialog">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-header">
          <h6 class="modal-title">Update Stok Logistik</h6>
        </div>
        <div class="modal-body">
          <form method="POST" action="{{url('asset_logistik/update_stok')}}" name="modalProgress">
            @csrf
...
<div class="modal fade" id="peminjaman_modal" role="dialog">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h6 class="modal-title">Request Logistik</h6>
        </div>
        <div class="modal-body">
          <div style="overflow: auto">
            <table id="product-add" class="table product-add">
              <tr class="tr-header">
                ...
                <th>Unit</th>
                <th>Description</th>
                <th><a href="javascript:void(0);" style="font-size:18px;" id="addMore" class="add"><span class="bx bx-plus"></span></a></th>
              </tr>
              <tr>
              <td style="margin-bottom: 50px;">
                <br><select class="form-control produk"  onchange="initlogistik()" name="logistik[]" id="logistik" data-rowid="0" style="font-size: 14px; width: 200px">
                ...
                <td>
                  <a href='javascript:void(0);'  class='remove'><span class="bx bx-x" style="font-size: 18px;margin-top: 20px;color: red;"></span></a>
                </td>
              </tr>
            </table>
            <div class="col-md-12 modal-footer" id="btn_submit">
              <br>
              <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-dismiss="modal"><i class="bx bx-x"> </i>&nbspClose</button>
              <button type="submit" id="btn_request" class="btn btn-sm btn-primary"><i class="bx bx-check"> </i>&nbspSubmit</button>
            </div>
          </div>
        </div>
      </div>
...
<div class="modal fade" id="request_modal" role="dialog">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <!-- <form method="POST" action="{{url('asset_logistik/store_request_logistik')}}" name="modalProgress"> -->
            <!-- @csrf -->
          <div style="overflow: auto">
            <table id="product-request" class="table product-add">
              <tr class="tr-header">
                ...
                <th>Description</th>
                <th>Link Product</th>
                <th><a href="javascript:void(0);" style="font-size:18px;" id="addMore2" class="add"><span class="bx bx-plus"></span></a></th>
              </tr>
              <tr>
                <td style="margin-bottom: 50px;">
                  <br><input class="form-control" name="logistik_request[]" placeholder="Enter Product Name" id="logistik_request" data-rowid="0" style="font-size: 14px; width: 200px" required>
                </td>
                <td style="margin-bottom: 50px;">
                  <br>
            ...
            <div class="col-md-12 modal-footer" id="btn_submit">
              <br>
              <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-dismiss="modal"><i class="bx bx-x"> </i>&nbspClose</button>
              <button type="submit" id="btn_request_asset" class="btn btn-sm btn-primary"><i class="bx bx-check"> </i>&nbspSubmit</button>
            </div>
          </div>
          <!-- </form> -->
        </div>
...
<div class="modal fade" id="accept_modal" role="dialog">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-header">
          <h6 style="text-align: center;"><b>Are you sure to accept?</b></h6>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>Nama Barang</label>
          ...
          <div class="modal-footer">
            ...
            <button type="submit" id="btn_accept_logistik" class="btn btn-sm btn-success"><i class="bx bx-check"></i>&nbsp Accept</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-body">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-sm btn-outline-secondary"  data-bs-dismiss="modal"><i class="bx bx-x"></i>&nbspCANCEL</button>
            <button type="submit" id="btn_reject_logistik" class="btn btn-sm btn-danger" ><i class="bx bx-check"></i>&nbsp Reject</button>
          </div>
        </form>
        </div>
      </div>
...
<div class="modal fade" id="reject_request_modal" role="dialog">
    <div class="modal-dialog modal-sm">
      <div class="modal-content">
        <div class="modal-body">
          <form method="POST" action="{{url('asset_logistik/reject_request_logistik')}}" name="modalProgress">
            @csrf
...
<div class="modal fade" id="done_modal" role="dialog">
 <div class="modal-dialog modal-sm">
    <div class="modal-content">
      <div class="modal-body">
        <form method="POST" action="{{url('asset_logistik/done_request_pr')}}" name="modalProgress">
          @csrf
        ...
        <div class="form-group">
          <h6 style="text-align: center;"><b>PR Done?</b></h6>
        </div>
        <div class="form-group"> 
          <label>Nama Barang</label>
...
<div class="modal fade" id="done_request_modal" role="dialog">
 <div class="modal-dialog modal-sm">
    <div class="modal-content">
      <div class="modal-body">
        <form method="POST" action="{{url('asset_logistik/done_request_logistik')}}" name="modalProgress">
          @csrf
        ...
        <div class="form-group">
          <h6 style="text-align: center;"><b>Request Done?</b></h6>
        </div>
        <div class="form-group"> 
          <label>Nama Barang</label>
        ...
        <div class="form-group">
          <label>Qty Restock</label>
          <input type="number" name="qty_restock_logistik" id="qty_restock_logistik" placeholder="Enter Quantity" class="form-control" required>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label>Unit</label>
          <select class="form-control unit_logistik" name="unit_request" id="unit_request"style="width: 100%" required>
            <option value="">Select Unit</option>
            @foreach($unit_assets as $unit_asset)
            <option value="{{$unit_asset->unit}}">{{$unit_asset->unit}}</option>
            @endforeach
...
@section('script')
  <script type="text/javascript">
    ...
    function reportExcel(data){
      $("#btnShowMonth").html(data.innerText + ' <span class="bx bx-caret-down"></span>')
    ...
    function exportExcel(url){
      ...
      if (month != '') {
        ...
        Swal.fire("<h6>Warning!!!</h6>", "<h6>Please Select Month First!</h6>")
      }
    }
    ...
    function initlogistik() {
      ...
      $.ajax({
        type:"GET",
        url: "asset_logistik/getAssetLogistik",
        success:function(result){
          ...
          $(".produk").select2({
            dropdownParent:$("#peminjaman_modal"),
    ...
    function id_accept_update(id_transaction,id_barang,qty,qty_akhir,nama_barang,keterangan,nik_peminjam,created_at){
      var swalAccept;
      var titleStatus = 'Accept Request Logistik'  
      ...
      $("#btn_accept_logistik").click(function(){
        if (qty == '0') {
          Swal.fire("<h6>Warning!!!</h6>", "<h6>Please Restock Quantity First!</h6>")
        } else {
          swalAccept = Swal.fire({
            ...
            if (result.value) {
              ...
              $.ajax({
                type:"GET",
                url:"{{url('asset_logistik/accept_request')}}",
                data:{
    ...
    $(document).on('click', '#btn_accept_request_logistik', function() {
      var id_barang = this.value
      $.ajax({
        type:"GET",
        url:'{{url("/asset_logistik/detail_produk_request")}}',
        data:{
      ...
      $("#btn_accept_request").click(function(){
        ...
        swalAccept.then((result) => {
          if (result.value) {
            ...
            $.ajax({
              type:"GET",
              url:"{{url('asset_logistik/accept_request_logistik')}}",
              data:{
                id_barang:id_barang
              },
              success: function(result){
    ...
    $('#btn_request').click(function() {
      ...
      var swalAccept;
      var titleStatus = 'Request Logistik'
      ...
      var val_id = $("select[name='logistik[]']").map(function(){
      ...
      if (!error) {
          ...
          if (result.value) {
            ...
            $.ajax({
              type:"POST",
              url:"{{url('asset_logistik/request_logistik')}}",
              data:{
                _token: "{{ csrf_token() }}",
                data: data,
              },
    ...
    $('#btn_request_asset').click(function() {
      ...
      var val_name = $("input[name='logistik_request[]']").map(function(){
        if ($(this).val() === "") {
          Swal.fire({
      ...
      if (!error) {
        swalAccept = Swal.fire({
          ...
          if (result.value) {
            ...
            $.ajax({
              type:"POST",
              url:"{{url('asset_logistik/store_request_logistik')}}",
              data:{
                _token: "{{ csrf_token() }}",
                data: data,
              },
    ...
    function done_request_logistik(id_barang,nama_barang,qty,nik,keterangan) {
    ...
    $(document).on('change',".produk",function(e) {
      var logistik = $('.produk').val();
      var rowid = $(this).attr("data-rowid");

         $.ajax({
          type:"GET",
          url:'asset_logistik/get_qty_logistik',
          data:{
            logistik:this.value,
          },
    ...
    function reject_update(id_transaction){
      var titleStatus = 'Reject Request Logistik'
      swalAccept = Swal.fire({
          title: titleStatus,
      ...
      swalAccept.then((result) => {
        if (result.value) {
          ...
          $.ajax({
            type:"GET",
            url:"{{url('asset_logistik/reject_request')}}",
            data:{
              id_transaction:id_transaction,
              reason:result.value
            },
    ...
    function reject_request_logistik(id_barang){
      var titleStatus = 'Reject Request Logistik'
      swalAccept = Swal.fire({
          title: titleStatus,
          text: "Reason for rejecting:",
      ...
      swalAccept.then((result) => {
        if (result.value) {
          ...
          $.ajax({
            type:"GET",
            url:"{{url('asset_logistik/reject_request_logistik')}}",
            data:{
              id_barang:id_barang,
              reason:result.value
            },
